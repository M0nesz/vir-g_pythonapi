  ...
  function view(data, request, id){
    ...
    if (request.status >= 200 && request.status < 400) {
         data.forEach((query) => {
          ...
          div.innerHTML = "<td>"+query.id+"</td><td><input id='name"+query.id+"' placeholder='"+query.name+"' value='"+query.name+"'/></td><td><input id='color"+query.id+"' placeholder='"+query.color+"' value='"+query.color+"'/></td><td><input id='size"+query.id+"' placeholder='"+query.size+"' value='"+query.size+"'/></td><td><input id='price"+query.id+"' placeholder='"+query.price+"' value='"+query.price+"'/></td><td><input id='shippingtime"+query.id+"' placeholder='"+query.shippingtime+"' value='"+query.shippingtime+"'/></td>"+"<button onclick = 'deleterecord("+query.id+")' type = 'submit' value='Submit'>Delete</button>"+"<button onclick = 'update("+query.id+")'>Update</button>" ;
...
function update(id){
  const data = JSON.stringify({
    ...
    shippingtime:document.getElementById("shippingtime"+id).value
